...
def pausar_bot(bot_id):
    ...
    try:
        cursor.execute("UPDATE bots SET ativo=0 WHERE id=%s AND user_email=%s", (bot_id, user_email))  # <-- CORRIGIDO AQUI
        conn.commit()
    except Exception as e:
